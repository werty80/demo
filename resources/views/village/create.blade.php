<x-layout>
    ...
    @endif
    <form method="POST" action="/villages">
        @csrf
        <div class="space-y-12">
            <div class="border-b border-gray-900/10 pb-12">
                <div class="sm:col-span-4">
                    ...
                    <div class="mt-2">
                        <select name="island_id" id="island" class="block w-full rounded-md bg-white pl-3 pr-10 py-1.5 text-gray-900 focus:outline-none sm:text-sm" required>
                            <option value="">Select an Island</option>
                            @foreach ($islands as $island)
                                <option
                                    value="{{ old('island_id', $island->id) }}"
                                    {{ $selectedIsland == $island->id ? 'selected' : '' }}
                                >
                                    {{ $island->name }}
                                </option>
                            @endforeach
